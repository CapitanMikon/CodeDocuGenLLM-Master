...
def f1_all_wrong(method_data):
    ...
    false_negative += 1  # ma mat popis a llm nedal popis
    false_negative += method_real_number_of_params  # ma params a llm nepridal
    false_negative += 1  # ma return value a llm nepridal return desc
    false_negative += total_throws  # ma throws a llm nepridal throws
...
def process_entry_all_wrong(javadoc_by_ai):
    return f1_all_wrong(javadoc_by_ai)
...
def run_calc_f1_scores(input_ai_dir):
    ...
    for path in json_file_paths_ai:
        ...
        for entry in ai_javadoc_data:
            e1 = [0, 0, 0, 0]
            if JAVADOC_GENER_ITERATION == "improved_javadoc" and entry[JAVADOC_GENER_ITERATION] == "INVALID FORMAT":
                e1 = process_entry_all_wrong(entry)
            else:
