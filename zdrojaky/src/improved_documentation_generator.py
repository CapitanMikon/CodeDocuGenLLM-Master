...
def process_json_file_with_path(path, repo):
    ...
    for obj in json_object:
        ...
        chain = create_simple_documentation_generation_chainB()

        print(
            f"\n ####### \n \tSignature: {obj['signature']} Lang: {language}")

        javadoc_with_method = obj['javadoc'] + obj['implementation'] if obj['javadoc'] != "INVALID FORMAT" else "" + obj['implementation']

        response = chain.invoke({"LANGUAGE": language, "METHOD": javadoc_with_method})
