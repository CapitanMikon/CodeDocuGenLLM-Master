...
def get_user_input():
    input_text = st.text_input("Human: ",
                               ...
                               key="input",
                               placeholder=f"Ask me for if you need any coding advice or code implementation.",
...
def remove_history():
    ...
    codegen_prototype.chat_history = []
...
def main():
    ...
    with st.sidebar:
        ...
        codegen_prototype.MODEL_NAME = st.selectbox(label="Model", options=codegen_prototype.AVAILABLE_MODELS)
        codegen_prototype.TEMPERATURE = st.slider("Temperature", 0.0, 1.0, 0.0)
    ...
    st.title(f"Personal coding bot with RAG:")
    ...
    if user_input := st.chat_input():
        ...
        with st.spinner("thinking..."):
            output = codegen_prototype.query_llm(user_input)
